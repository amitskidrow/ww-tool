...
class WeDashApp(App):
    ...
    async def action_focus_search(self) -> None:
        ...
        try:
            ...
            search.focus()
            try:
                # Select all if supported by current Textual version
    ...
    async def action_focus_terminal(self) -> None:
        # Toggle Terminal view; lazy-create a per-service terminal placeholder
        if not self.term_container:
            return
        svc = self._selected_service()
        if not svc:
            return
        # First time: create and mount the terminal widget for this service
    ...
    async def action_term_sigint(self) -> None:
        # Send Ctrl-C to active terminal session if visible
        if not self._show_terminal:
            return
        svc = self._selected_service()
        if not svc:
    ...
    async def action_term_clear(self) -> None:
        ...
        try:
            ...
            pass
        try:
            # Fallback to running 'clear'
            _ = sess.run_argv(["clear"], cwd=svc.dir)
        except Exception:
            pass

    @on(Tabs.TabActivated)
    def _on_tabs_changed(self, event: Tabs.TabActivated) -> None:
        # Header filter tabs have id="tabs"; terminal tabs have id="term-tabs"
        tabs_id = getattr(event, "tabs", None)
        tabs_id = getattr(tabs_id, "id", None) or getattr(event.control, "id", None)
        if tabs_id == "tabs":
            ...
            # Switch active terminal panel by key
            key = event.tab.id or ""
            try:
                ...
                pass
            try:
                self._maybe_enable_tmux_refresh(key)
            except Exception:
                pass

    def _ensure_terminal_for_service(self, svc: Service, activate: bool = False) -> None:
        # Create session and tabs/switcher child if needed
        if not self.term_container:
            return
        sess = self._terminals.get_or_create(svc.unit, cwd=svc.dir)
        # Add a Tab if missing
        if svc.unit not in self._term_tabs:
            tabs = self.query_one('#term-tabs', Tabs)
            tab = Tab(svc.name or svc.unit, id=svc.unit)
            tabs.add_tab(tab)
            self._term_tabs[svc.unit] = tab
        # Add the widget to ContentSwitcher if not present
        switcher = self.query_one('#term-switcher', ContentSwitcher)
        if sess.widget.parent is None:
            try:
                sess.widget.id = svc.unit  # type: ignore[attr-defined]
            except Exception:
                pass
            switcher.mount(sess.widget)
        if activate:
            try:
                tabs = self.query_one('#term-tabs', Tabs)
                tabs.active = svc.unit
                switcher.current = svc.unit
            except Exception:
                pass
            try:
                self._maybe_enable_tmux_refresh(svc.unit)
    ...
    async def action_term_next(self) -> None:
        # Cycle to the next terminal tab if visible
        if not self._show_terminal:
            return
        try:
            tabs = self.query_one('#term-tabs', Tabs)
            # Gather tab ids in order
    ...
    async def action_term_prev(self) -> None:
        # Cycle to the previous terminal tab if visible
        if not self._show_terminal:
            return
        try:
            tabs = self.query_one('#term-tabs', Tabs)
...
def parse_args(argv: Iterable[str] | None = None) -> argparse.Namespace:
    ...
    p.add_argument("--terminal-backend", choices=["auto", "native", "tmux"], default="auto", help="Terminal backend selection")
...
def main(argv: Iterable[str] | None = None) -> None:
    ...
    app = WeDashApp(state=state, last=ns.last, columns=ns.columns, terminal_backend=ns.terminal_backend)
    app.run()
